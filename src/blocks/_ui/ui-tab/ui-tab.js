export default function uitab() {
  ...
  for (const tabBlock of tabBlocks) {
    ...
    const headItems =
      headGroup.length > 0
        ? document.querySelectorAll(`.ui-tab-head[data-tab-group="${tabGroup}"] > .ui-tab-head__item`)
        : tabBlock.querySelectorAll('.ui-tab-head__item');
    const bodyItems =
      bodyGroup.length > 0
        ? document.querySelectorAll(`.ui-tab-body[data-tab-group="${tabGroup}"] > .ui-tab-body__item`)
        : tabBlock.querySelectorAll('.ui-tab-body__item');
